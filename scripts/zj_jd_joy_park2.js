/**
 * hellow 仓库 ts 改 js 直接执行 自用版本
 */
const axios = require("axios");
const USER_AGENT = require("./JS_USER_AGENTS");
...
function api(fn, body) {
  return new Promise(async (resolve) => {
    let { data } = await axios.post(
      "https://api.m.jd.com/",
      `functionId=${fn}&body=${JSON.stringify(
        body
...
function joyList() {
  return new Promise(async (resolve) => {
    let { data } = await axios.get(
      `https://api.m.jd.com/?functionId=joyList&body={%22linkId%22:%22LsQNxL7iWDlXUs6cFl-AAg%22}&_t=${Date.now()}&appid=activities_platform`,
      {
        headers: {
          host: "api.m.jd.com",
          "User-agent": USER_AGENT,
...
function merge() {
  return new Promise(async (resolve) => {
    ...
    if (mergeTemp.length >= 2) {
      console.log("aaa");
      res = await api("joyMerge", {
        joyOneId: mergeTemp[0].id,
        joyTwoId: mergeTemp[1].id,
        linkId: "LsQNxL7iWDlXUs6cFl-AAg",
      });
      console.log(res);
    } else if (mergeTemp.length === 1) {
      console.log("bbb");
      res = await api("joyBuy", {
        level: level,
        linkId: "LsQNxL7iWDlXUs6cFl-AAg",
      });
    ...
    let jid1 = res.data.id
    console.log(jid1)
    await wait(2000)
    res = await api('joyBuy', {"level": level, "linkId": "LsQNxL7iWDlXUs6cFl-AAg"})
    let jid2 = res.data.id
    console.log(jid2)
    await wait(2000)
